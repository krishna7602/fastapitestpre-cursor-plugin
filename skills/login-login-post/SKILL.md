...
## When to use
Use this skill when the user intent maps to `login-login-post` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `login-login-post` and list required vs optional parameters.
...
4. Call the `login-login-post` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `login-login-post` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `login-login-post`, extract the top-level summary fields.
