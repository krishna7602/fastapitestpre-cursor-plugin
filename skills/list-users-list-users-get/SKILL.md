---
name: list-users-list-users-get
...
## When to use
Use this skill when the user intent maps to `list-users-list-users-get` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `list-users-list-users-get` and list required vs optional parameters.
...
4. Call the `list-users-list-users-get` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `list-users-list-users-get` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `list-users-list-users-get`, extract the top-level summary fields.
