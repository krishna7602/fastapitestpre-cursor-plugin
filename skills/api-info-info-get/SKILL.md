...
## When to use
Use this skill when the user intent maps to `api-info-info-get` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `api-info-info-get` and list required vs optional parameters.
...
4. Call the `api-info-info-get` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `api-info-info-get` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `api-info-info-get`, extract the top-level summary fields.
