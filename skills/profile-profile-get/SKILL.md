...
## When to use
Use this skill when the user intent maps to `profile-profile-get` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `profile-profile-get` and list required vs optional parameters.
...
4. Call the `profile-profile-get` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `profile-profile-get` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `profile-profile-get`, extract the top-level summary fields.
