...
## When to use
Use this skill when the user intent maps to `register-register-post` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `register-register-post` and list required vs optional parameters.
...
4. Call the `register-register-post` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `register-register-post` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `register-register-post`, extract the top-level summary fields.
