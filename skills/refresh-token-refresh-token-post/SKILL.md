---
name: refresh-token-refresh-token-post
...
## When to use
Use this skill when the user intent maps to `refresh-token-refresh-token-post` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `refresh-token-refresh-token-post` and list required vs optional parameters.
...
4. Call the `refresh-token-refresh-token-post` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `refresh-token-refresh-token-post` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `refresh-token-refresh-token-post`, extract the top-level summary fields.
