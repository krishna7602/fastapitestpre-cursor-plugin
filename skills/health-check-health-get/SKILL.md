...
## When to use
Use this skill when the user intent maps to `health-check-health-get` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `health-check-health-get` and list required vs optional parameters.
...
4. Call the `health-check-health-get` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `health-check-health-get` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `health-check-health-get`, extract the top-level summary fields.
